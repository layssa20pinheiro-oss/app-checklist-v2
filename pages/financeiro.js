...
export default function FinanceiroNegocio() {
  ...
  const transacoesMensais = transacoes.filter(t => {
    ...
    // Mostra este mês OU tudo que ficou pra trás (pendentes/atrasados)
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#5a5b5b] to-[#7e7f7f] font-sans pb-10 px-6">
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="space-y-3">
          {loading ? (
            <p className="text-center py-20 text-white/20 uppercase text-[9px] tracking-widest animate-pulse">Carregando...</p>
          ) : (
            <>
              {listaExibida.map(t => (
              ...
              {listaExibida.length === 0 && (
                <p className="text-center py-20 text-white/20 uppercase text-[9px] tracking-widest font-bold">Sem lançamentos para este período</p>
              )}
            </>
          )}
        </div>
