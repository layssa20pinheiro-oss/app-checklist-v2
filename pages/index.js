...
export default function ChecklistApp() {
  ...
  // BLOCO 1: Lógica de Edição (Pega o id do histórico)
  useEffect(() => {
  ...
  const salvarRelatorio = async () => {
    ...
    try {
      ...
      // 3. SALVA NO BANCO (Update ou Insert)
  ...
  // BLOCO 3: Enviar WhatsApp (O TEXTO EXATO DA FOTO)
  ...
  return (
    <div className="min-h-screen bg-[#7e7f7f] p-4 flex flex-col items-center font-sans text-slate-800 pb-10">
      ...
      {/* Voltei a URL absoluta do Supabase para garantir o fundo cinza da Imagem_11 */}
      <Head>
      ...
      {/* Logo no topo do App (Usando icon.png local para dentro do app) */}
      <img src="/icon.png" className="max-w-[140px] mb-10 mt-6" alt="Logo" />

      {/* TELA 1: FORMULÁRIO (TELA DA SUA FOTO) */}
      {etapa === 'form' && (
      ...
      {etapa === 'sucesso' && (
        <div className="bg-white rounded-[45px] p-12 text-center shadow-2xl max-w-xs mt-20 animate-in zoom-in duration-500">
          ...
          <button onClick={() => window.location.href='/'} className="w-full text-gray-400 py-4 text-[10px] font-bold uppercase tracking-widest hover:text-gray-600">Novo Checklist</button>
        </div>
      )}
