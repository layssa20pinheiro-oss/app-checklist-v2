...
import { Camera, Plus, Trash2, Send, Loader2, ClipboardList, History, ArrowLeft, ExternalLink } from 'lucide-react';
...
export default function ChecklistApp() {
  const [etapa, setEtapa] = useState('form'); // Já começa direto no formulário
  ...
  // Caso alguém abra o link com ?id=, ele mostra o relatório
  useEffect(() => {
  ...
  const salvarRelatorio = async () => {
    ...
    setLoading(true);
    try {
      // 1. Gerar imagem do relatório para o cliente
      ...
      // 2. Se houver foto dos pertences, faz o upload dela separada
  ...
  return (
    <div className="min-h-screen bg-[#7e7f7f] p-4 flex flex-col items-center font-sans text-slate-800 pb-10">
      <Head>
        <title>Cerimonial Elite | Checklist</title>
      ...
      {/* --- FORMULÁRIO (TELA DA SUA FOTO) --- */}
      {etapa === 'form' && (
        <div className="w-full max-w-md animate-in fade-in duration-500">
           <div className="flex justify-end mb-2">
              <button onClick={() => window.location.href='/historico'} className="text-white/40 text-[10px] uppercase font-bold flex items-center gap-1">
                Ver Gerenciamento <ExternalLink size={10}/>
              </button>
           </div>
      ...
      {etapa === 'sucesso' && (
        <div className="bg-white rounded-[45px] p-12 text-center shadow-2xl max-w-xs mt-20 animate-in zoom-in duration-500">
          ...
          <button onClick={() => setEtapa('form')} className="w-full text-gray-400 py-4 text-[10px] font-bold uppercase tracking-widest">Novo Checklist</button>
        </div>
      )}

      {/* --- VISUALIZAÇÃO DO CLIENTE --- */}
      {etapa === 'view' && (
        <div className="w-full flex flex-col items-center animate-in fade-in duration-500 mt-10">
